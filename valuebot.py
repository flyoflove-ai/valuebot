...
MODEL_BLOCK = re.compile(
    r"(tts|live|embedding|embed|aqa|transcribe|image|imagen|veo|nano-banana|robotics|"
    r"vision-only|gemini-1\.|gemini-2\.0|gemini-2\.5)",
    re.I,
)
...
def build_candidates():
    """ListModels 결과와 선호 순서를 교차해 후보 리스트 생성.
    주의: ListModels는 이미 신규 차단된 모델도 반환한다. 목록은 참고만 하고
    최종 판단은 probe_model()의 실제 호출로 한다."""
    if FORCED_MODEL:
        return [FORCED_MODEL]
    ...
    try:
        ...
        log("ListModels failed:", e)

    cands = []
    for pref in MODEL_PREF:                      # 1순위: 선호목록 ∩ 실제목록
        if pref in listed and pref not in cands:
            cands.append(pref)
    for pref in MODEL_PREF:                      # 2순위: 선호목록 (별칭은 목록에 없을 수 있음)
        if pref not in cands:
            cands.append(pref)
    for name in sorted(listed, reverse=True):    # 3순위: 목록의 나머지
...
def probe_model(name):
    """실제 generateContent를 1토큰 쏴서 살아있는지 확인."""
    try:
        r = requests.post(
            f"{GEM_BASE}/models/{name}:generateContent?key={GEMINI_KEY}",
...
def select_model(exclude=None):
    """후보를 순회하며 첫 번째로 살아있는 모델 선택."""
    exclude = exclude or set()
    for name in STATE["candidates"]:
        if name in exclude or name in STATE["dead"]:
            continue
        ok, why = probe_model(name)
        if ok:
            log(f"model OK: {name} ({why})")
            return name
        log(f"model dead: {name} -> {why}")
        STATE["dead"].add(name)
    return None
...
def handle(msg):
    ...
    if text.startswith("/model"):
        arg = text[6:].strip()
        if arg:
            ok, why = probe_model(arg)
            if ok:
                STATE["model"] = arg
                STATE["dead"].discard(arg)
                send(chat_id, f"✅ 모델 변경: <code>{arg}</code>")
            else:
                send(chat_id, f"❌ <code>{arg}</code> 사용 불가\n<code>{why}</code>")
            return
        alive = [c for c in STATE["candidates"] if c not in STATE["dead"]][:6]
        msg = f"현재 모델: <code>{STATE['model']}</code>\n\n대기 후보:\n"
        msg += "\n".join(f" · {c}" for c in alive)
        if STATE["dead"]:
            msg += "\n\n제외됨(404):\n" + "\n".join(f" · {d}" for d in sorted(STATE["dead"]))
        msg += "\n\n변경: <code>/model gemini-3.1-pro-preview</code>"
        send(chat_id, msg)
        return
...
def main():
    ...
    STATE["candidates"] = build_candidates()
    log("candidates:", ", ".join(STATE["candidates"][:8]))
    STATE["model"] = select_model()
    if not STATE["model"]:
        log("치명적: 호출 가능한 모델이 하나도 없습니다. API 키를 확인하세요.")
        sys.exit(1)
